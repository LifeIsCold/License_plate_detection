...
det_model = torch.hub.load('ultralytics/yolov5', 'custom', path='plate_detection.pt')
...
def process_images(input_dir="car_images", output_dir="plates_trocr"):
    ...
    for idx, fname in enumerate(os.listdir(input_dir)):
        ...
        for i, (*box, conf, cls) in enumerate(results.xyxy[0]):
            if results.names[int(cls)] != 'plate' or conf < 0.8:
                continue

            x1, y1, x2, y2 = map(int, box)
            pad = -10  # Negative padding to crop inside
            h, w = img.shape[:2]
            ...
            plate_crop = remove_white_border(plate_crop)
            plate_filename = f"plate_{idx}_{i}.jpg"
            cv2.imwrite(os.path.join(output_dir, plate_filename), plate_crop)
            ...
            print(f"[{plate_filename}] -> {final_plate_text}")
